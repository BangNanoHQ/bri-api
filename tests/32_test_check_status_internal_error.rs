...
#[tokio::test]
async fn test_check_status_internal_error() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Create check status request that will trigger internal error
    let request = CheckStatusRequest {
        original_partner_reference_no: "22222".to_string(), // Reference that triggers internal error
    ...
    assert!(err_string.contains("5008201"));
