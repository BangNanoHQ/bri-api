...
#[tokio::test]
async fn test_inquiry_invalid_format() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let request = InquiryRequest {
        beneficiary_bank_code: "CENAIDJAO".to_string(), // Invalid format - too long
        beneficiary_account_no: "1234567890".to_string(),
    };
    ...
    assert!(err_string.contains("4008101"));
    assert!(err_string.contains("Invalid Field Format beneficiaryBankCode"));
