...
#[tokio::test]
async fn test_check_status_general_error() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Create check status request that will trigger general error
    let request = CheckStatusRequest {
        original_partner_reference_no: "33333".to_string(), // Reference that triggers general error
    ...
    assert!(err_string.contains("5008200"));
    assert!(err_string.contains("General Error"));
