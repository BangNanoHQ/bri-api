...
#[tokio::test]
async fn test_transfer_unauthorized() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Create transfer request with account that will trigger unauthorized
    let request = TransferRequest {
        customer_reference: "54321".to_string(),
        sender_identity_number: "3515085211930002".to_string(),
        source_account_no: "66666".to_string(), // Account number that triggers unauthorized
        amount: Amount {
    ...
    assert!(err_string.contains("4018000"));
