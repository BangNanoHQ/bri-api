...
#[tokio::test]
async fn test_check_status_not_found() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Create check status request with non-existent transaction
    let request = CheckStatusRequest {
        original_partner_reference_no: "17010".to_string(), // Reference that doesn't exist
    ...
    assert!(err_string.contains("4048200"));
    assert!(err_string.contains("Invalid Transaction Status"));
