...
#[tokio::test]
async fn test_transfer_conflict() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let request = TransferRequest {
        customer_reference: "99999".to_string(),
    ...
    assert!(err_string.contains("4098000"));
    assert!(err_string.contains("Conflict"));
