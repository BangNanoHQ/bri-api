...
#[tokio::test]
async fn test_check_status_timeout() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Create check status request that will trigger timeout
    let request = CheckStatusRequest {
        original_partner_reference_no: "44444".to_string(), // Reference that triggers timeout
    ...
    assert!(err_string.contains("5048200"));
