...
#[tokio::test]
async fn test_check_status_invalid_format() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Create check status request with invalid format
    let request = CheckStatusRequest {
        original_partner_reference_no: "645432439999999999999".to_string(), // Too long reference number
    ...
    assert!(err_string.contains("4008201"));
    assert!(err_string.contains("Invalid Field Format originalPartnerReferenceNo"));
