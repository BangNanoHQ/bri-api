...
#[tokio::test]
async fn test_transfer_timeout() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Create transfer request that will trigger timeout
    let request = TransferRequest {
        customer_reference: "44444".to_string(), // Reference that triggers timeout
    ...
    assert!(err_string.contains("5048000"));
