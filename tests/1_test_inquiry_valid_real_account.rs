...
#[tokio::test]
async fn test_inquiry_valid_real_account() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(!response.receiver_name.is_empty());
    assert_eq!(response.beneficiary_bank_code, "CENAIDJA");
    assert_eq!(response.beneficiary_account_no, "99999");
    assert_eq!(response.beneficiary_account_type, "SVGS");
    assert!(!response.account_number.is_empty());
    assert_eq!(response.receiver_type, "02");
    assert_eq!(response.receiver_resident_status, "01");
    assert!(!response.receiver_identity_number.is_empty());
