...
#[tokio::test]
async fn test_transfer_internal_error() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Create transfer request that will trigger internal error
    let request = TransferRequest {
        customer_reference: "22222".to_string(), // Reference that triggers internal error
    ...
    assert!(err_string.contains("5008001"));
