...
#[tokio::test]
async fn test_check_status_unauthorized() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Create check status request that will trigger unauthorized error
    let request = CheckStatusRequest {
        original_partner_reference_no: "66666".to_string(), // Reference that triggers unauthorized error
    ...
    assert!(err_string.contains("4018200"));
    assert!(err_string.contains("Unauthorized. Client"));
