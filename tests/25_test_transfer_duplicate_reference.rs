...
#[tokio::test]
async fn test_transfer_duplicate_reference() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Create transfer request with duplicate customer reference
    let request = TransferRequest {
        customer_reference: "123456".to_string(), // Using duplicate reference
        sender_identity_number: "3515085211930002".to_string(),
        source_account_no: "44444".to_string(), // Account that triggers duplicate reference error
        amount: Amount {
    ...
    assert!(err_string.contains("4098001"));
    assert!(err_string.contains("Duplicate customerReference"));
